...
const fieldCommands = Object.assign({}, ...Object.entries({...CHARS.FIELD_CONTROL, ...CHARS.FIELD_SPECIAL}).map(([a,b]: any) => ({ [b]: a })))

export const decodeText = function(buf: Buffer): Buffer {
    let text = '';
    let i = 0;
    ...
    while (i < buf.length) {
        ...
        // Field control code
            ...
            // Other control codes
                ...
                if (!CHARS.FIELD_CONTROL[k]) throw Error (`Illegal control code ${k}`)
                text += CHARS.FIELD_CONTROL[k]
            }
        }

        // Field special character
        else {
            text += CHARS.FIELD_SPECIAL[c];
            if (c === 0xE8) text += "\n";
        }
    }
...
export const encodeText = function(text: string): Buffer {
    let data = [];
    let i = 0;
    ...
    while (i < text.length) {
        ...
        // command sequence
            ...
            if (keyword === 'WAIT') {
                ...
                data.push(0xFE, 0xDD, swap16(arg))
            }
            else if (keyword === 'STR') {
                ...
                data.push(0xFE, 0xE2, swap16(offset), swap16(length))
            }
            else {
